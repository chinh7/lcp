...
func (*Service) parseParam(param *abi.Parameter, value []byte) (string, error) {
	switch param.Type {
	case abi.Address:
...
func (service *Service) parseFunction(methodID crypto.MethodID, args []byte, contract *abi.Contract) (*call, error) {
	if contract == nil {
		return nil, nil
	}
	...
	for i, arg := range parsedArgs {
		...
		value, err := service.parseParam(param, arg)
		if err != nil {
			return nil, err
		}
...
func (service *Service) parseTransaction(tx *crypto.Transaction, blockHeight uint64) (*transaction, error) {
	...
	if tx.Payload.ID != (crypto.MethodID{}) {
		parsedPayload, err := service.parseFunction(tx.Payload.ID, tx.Payload.Args, contract)
		if err != nil {
			return nil, err
		}
