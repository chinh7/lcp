...
func TestApplyTx(t *testing.T) {
	...
	data, err := util.BuildDeployTxData("../test/testdata/liquid-token.wasm", "../test/testdata/liquid-token-abi.json", "", []string{})
	if err != nil {
		panic(err)
	}
	deployTx := &crypto.Tx{From: signer, Data: data, To: crypto.Address{}, GasLimit: 0, GasPrice: 0}

	contractWithInitTxData, err := util.BuildDeployTxData("../test/testdata/liquid-token.wasm", "../test/testdata/liquid-token-abi.json", InitFunctionName, []string{"100"})
	if err != nil {
		panic(err)
	}
	...
	invokeData, err := util.BuildInvokeTxData("../test/testdata/liquid-token-abi.json", "mint", []string{"1000"})
	if err != nil {
		panic(err)
	}
	invokeTx := &crypto.Tx{
	...
	invalidInvokeData, err := util.BuildInvokeTxData("../test/testdata/liquid-token-abi.json", "mint", []string{"1000"})
	if err != nil {
		panic(err)
	}
