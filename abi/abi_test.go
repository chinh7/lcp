...
func TestBytesEncoding(t *testing.T) {
	...
	testTables := []struct {
		...
		{
			types:   parameters1,
			encoded: []byte{88, 142, 133, 178, 236, 112, 199, 209, 176, 108, 122, 171, 33, 245, 148, 112, 199, 6, 6, 52, 233, 179, 85, 154, 228, 40, 94, 109, 248, 245, 224, 151, 44, 51, 238, 88, 213, 168, 54, 126, 174, 198, 26, 35, 156, 150, 103, 161, 151, 44, 88, 225, 16, 235, 121, 255, 255, 84, 24, 251, 255, 255, 255, 255, 255, 133, 5, 2, 70, 81, 107, 154, 7, 43, 124, 80, 193},
		...
		{
			types:   parameters2,
			encoded: []byte{2, 0, 88, 142, 133, 178, 236, 112, 199, 209, 176, 108, 122, 171, 33, 245, 148, 112, 199, 6, 6, 52, 233, 179, 85, 154, 228, 40, 94, 109, 248, 245, 224, 151, 44, 51, 238, 88, 142, 133, 178, 236, 112, 199, 209, 176, 108, 122, 171, 33, 245, 148, 112, 199, 6, 6, 52, 233, 179, 85, 154, 228, 40, 94, 109, 248, 245, 224, 151, 44, 51, 238, 2, 0, 88, 255, 2, 0, 43, 2, 45, 48, 3, 0, 54, 126, 174, 198, 54, 126, 174, 198, 54, 125, 252, 1, 2, 0, 26, 35, 156, 150, 103, 161, 151, 44, 59, 128, 234, 1, 0, 0, 0, 0, 2, 0, 88, 168, 2, 0, 77, 1, 226, 253, 2, 0, 34, 169, 0, 0, 104, 24, 251, 255, 2, 0, 26, 143, 154, 254, 255, 255, 255, 255, 135, 239, 58, 136, 2, 0, 0, 0, 2, 0, 117, 167, 167, 196, 60, 60, 69, 71, 2, 0, 160, 26, 47, 221, 169, 39, 0, 193, 174, 71, 225, 193, 251, 14, 136, 65},
	...
	for _, table := range testTables {
		...
		if diff := cmp.Diff(decoded, table.decoded); diff != "" {
			t.Errorf("Decoding of %v is incorrect, expected: %v, got: %v, diff: %v", table.encoded, table.decoded, decoded, diff)
		}
		encoded, err := EncodeFromBytes(table.types, decoded)
		if diff := cmp.Diff(encoded, table.encoded); diff != "" {
			t.Errorf("Encoding of %v is incorrect, expected: %v, got: %v, diff: %v", decoded, table.encoded, encoded, diff)
		}
	}
}
